...
`prepare_cell_dataset.py` builds a Zarr dataset for cell segmentation training and test from:
...
_crop_patch(img2d, cy, cx, half)  
Extracts a square patch centered at `(cy, cx)`.

_hist_feat(patch)  
Computes a normalized histogram feature vector.

_cosine(a, b)  
Computes cosine similarity between histogram vectors.

add_ellipse(mask2d, top, left, h, w)  
Draws an ellipse ROI into a mask.

build_union_mask(cells, mask)  
Builds the initial mask volume from ROI annotations.

propagate_cell_labels_by_hist(...)  
Propagates cell labels to neighboring slices.

build_dataset(folder, zarr_name)  
Processes all ROI/TIFF pairs and builds the Zarr dataset.
...
The output dataset name is controlled by the variable `zarr_name` in the script:
