...
train_trfm = Compose([
    SpatialPadd(keys=["image", "label", "valid"], spatial_size=(D, PH, PW), mode="constant"),
    ScaleIntensityRangePercentilesd(keys="image", lower=1, upper=99, b_min=0.0, b_max=1.0, clip=True),
    RandCropByPosNegLabeld(
        keys=["image", "label", "valid"],
        label_key="label",
        spatial_size=(D, PH, PW),
        pos=1, neg=1,
        num_samples=1,
    ),
    RandFlipd(keys=["image", "label", "valid"], prob=0.5, spatial_axis=1), # H fliping
    RandFlipd(keys=["image", "label", "valid"], prob=0.5, spatial_axis=2), # W fliping
...
def masked_bce_dice_loss(logits, target, valid, eps=1e-6):
    # valid weights between 0 and 1. 0 means unknown
    # size of inputs are (B, 1, S, H, W) 
...
student, teacher = train_mean_teacher(
    ...
    checkpoint_path=r"C:\Users\bmahsa\Downloads\PreR_AllSamples\training_valid_zero",
    available_checkpoint=r"C:\Users\bmahsa\Downloads\In Vivo Image_Wholemount (CART Samples)\train checkpoints\checkpoint_epoch_299.pth"
)
